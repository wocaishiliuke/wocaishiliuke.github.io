...
> 这里HandlerMapping和HandlerAdaptor可以省略（会默认配置）
...
> - 当是/（规则7时），优先级比默认配置的*.jsp（规则3）低，则会被*.jsp拦截，进而匹配到
...
> 这里指定了默认的HandlerMapping 、HandlerAdapter和ViewResolver等，如何没有指定这些对应的配置，那么在启动时这里的类都会被创建实例，所以之前的2个配置可以缺省，因为这里已经配置了缺省
...
> 同XML版有不同，这里使用默认的SpringMVC配置文件路径
