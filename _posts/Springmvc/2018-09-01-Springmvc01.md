...
> web.xml和Servlet版本的对应关系参考[Java Web中的版本](http://blog.wocaishiliuke.cn/servlet/2018/10/20/Servlet-note01/)
...
> 这里HandlerMapping和HandlerAdaptor可以省略（会默认配置）。**Spring4.0后，如果不显式配置HandlerMapping、HandlerAdapter、ViewResolver，会使用默认配置，详见DispatcherServlet.properties**
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <!-- XML版默认配置的HandlerMapping和HandlerAdaptor，可缺省-->
...
> - 有些组件只允许一个实例，如LocaleResolver和MuliipartResolver（不显式配置就不加载该组件），有些允许多个实例，如HandlerMapping 、HandlerAdapter、ViewResolver
