...
- 当方法参数为非封装基本类型（long），则url参数必须存在，否则报错
- 当请求参数（id=abc）和方法参数(Long id)类型不一致，会抛404 MethodArgumentTypeMismatchException异常

> http://localhost:9091/param/test2?username=haha&id=123
> http://localhost:9091/param/test2?username=haha
> http://localhost:9091/param/test2?username=haha&id=abc
...
> - 请求该接口方法，http://localhost:9091/param/hello
...
- [官方文档，记得找对应版本](https://docs.spring.io/spring/docs/current/spring-framework-reference/web.html)