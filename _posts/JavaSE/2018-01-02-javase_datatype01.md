...
> Java中分为局部变量、成员变量。成员变量又可分为静态成员变量和非静态成员变量。

#### 1.局部变量

> - 声明在方法上或方法中，存在于栈内存，生命周期随方法
> - 局部变量 **使用前**必须赋值(只声明不使用，不报错)
...
- 1.类名直接访问：类名.变量名
- 2.通过实例访问：被private修饰的用getter()，非私有的也可以实例.变量名

> 静态变量是全局的，甚至有些可以整个项目共享
