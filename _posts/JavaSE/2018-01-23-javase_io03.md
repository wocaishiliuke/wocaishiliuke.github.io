...
本文拷贝自[Java编程拾遗『字符流』](http://lidol.top/java/1274/)，讲述字节流的相关内容。
...
任何Reader实现类都可以一次读取多个字符，放入字符数组中。那么需要预先定义用来存放读取内容的字符数组大小，如果长度不够，不方便扩展。此时就可以借助CharArrayWriter，先通过Reader将输入流中的内容读取到一个固定大小的字符数组中，如果超过固定大小的字符数组，可分多次读取，然后将每次读取的内容依次写入到CharArrayWriter对象中，最后调用CharArrayWriter.toCharArray()获取一个完整的数组。
...
如果CharArrayWriter的char数组容量不够，会进行动态扩容（两倍）。除构造外，CharArrayWriter重写了Writer接口的方法。
