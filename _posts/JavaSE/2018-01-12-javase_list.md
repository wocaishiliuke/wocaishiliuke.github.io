...
|:---|:----|:-----|:---------|
|数据结构|数组|数组|链表|
|效率|按索引查询快O(1)，增删慢O(N)|按索引查询快O(1)，增删慢O(N)|不支持随机访问；按索引查询慢（从头/尾找，平均O(N/2)）；增删需要先定位，但不需要移位，修改本身很快O(1)|
|线程|线程不安全，效率高|线程安全，效率低|线程不安全，效率高|

> 这里ArrayList和Vector的查询指的是，使用索引访问。两者根据值查找元素的效率比较低，未排序时，平均时间复杂度为O(N/2)，已排序时，可以通过二分查找，时间复杂度为O(logN)。三者根据值查找，都依赖equals()。

当容器需要大量插入删除操作时，避免使用ArrayList，可选用LinkedList。

> 由于存在把线程不安全转变成线程安全的工具类Collections，即使Vector是线程安全的，也没能避免被ArrayList替代
...
- 实现了RandomAccess，支持随机访问（因为物理存储连续），索引访问时间复杂度O(1)（constant time）
- 支持序列化
- 可以被克隆
...
- 第一次add时，经过calculateCapacity({},1)后，执行ensureExplicitCapacity(10)
...
ArrayList虽然不需要保证元素唯一，但它的contains(Object o)和remove(Object o)，需要先根据值查找元素，依赖元素的equals()。（根据元素值进行的操作，都依赖元素的equals）
...
LinkedList是List基于**双向链表**的实现，集合中的每个元素都是链表中的一个节点。元素逻辑上连续，物理存储上不连续，不支持RandomAccess。
...
LinkedList的索引跟ArrayList的索引不同：
- ArrayList的index是物理地址偏移量，可用于随机访问
- LinkedList的index是虚拟的。first节点的index=0，它的next指向的下一节点index=1，直到最后的last节点的index=size-1

LinkedList的index不支持随机访问，只用于Node(index)查找节点时，决定从头还是从尾开始检索。

另外，LinkedList实现了Deque接口（Double Ended Queues），即可以操作双端队列。而链表本身可以实现队列和栈，所以LinkedList可用作栈、队列、双向队列。
...
**node(int index)根据索引查找。如果索引位于前半部分（index<(size>>1)），从头开始顺藤摸瓜，否则从尾开始，稍微提高了效率**。同时也验证了之前的结论，ArrayList支持随机访问，查询效率比较高，LinkedList虽然有索引，但物理存储不连续，不支持随机访问，需要从头或尾遍历，效率相对较低。
...
['dek]，Deque是个双向队列，定义了支持栈和队列的相关方法，所以可以使用LinkedList实现栈、队列（单向、双向）的相关操作。Deque继承自单向队列Queue，先来看下Queue接口。
...
**栈**是一种FILO数据结构，只操作头部。Deque接口中定义了一些栈相关的操作方法

> 源自JDK 1.0的java.util.Stack（继承自Vector）就是栈这一数据结构在Java中的实现，但现在很少使用。

**队列**是一种FIFO数据结构，两端都操作。但尾部只添加、头部只查看和删除。但Deque更为通用，可以操作两端。即除了上述栈操作方法，Deque还支持双向队列操作。
...
LinkedList有以下4种获取迭代器的方式。跟ArrayList相似，也有并发修改异常。

```java
//1.继承自AbstractSequentialList，返回ListItr
public Iterator<E> iterator() {
    return listIterator();
}
