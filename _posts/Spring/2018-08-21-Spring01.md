...
[Spring](https://spring.io/)是分层的、JavaSE/EE一站式(full-stack)、轻量级开源框架。这里基于[Spring Framework 4.3.22](https://docs.spring.io/spring/docs/4.3.22.RELEASE/spring-framework-reference/htmlsingle/#beans-introduction)讲解。
...
将对象的创建和管理交给Spring工厂。由Book book = new Book()变为Book book = Spring工厂.getBook()。

> **IoC强调将对象的创建交由IoC容器；DI更强调IoC容器把对象间的依赖关系也维护了**，两者实际是相同的概念。
...
> 对上例，通过下面的依赖注入改造，就可以省去在UserServiceImpl中创建context，然后getBean。将UserService和UserDao的依赖关系交由Ioc容器管理，并在创建UserService实例后就注入UserDao。
...
对于User：
- 要有空参构造和对应3个属性的setter（setter不能省）
...
> 这里该bean要能访问到jdbc.username，即Spring引入了jdbc.properties，可以获取到键值对
...
> 这里Test要能访问到userDao和dataSource.url，即userDao和dataSource也在该bean所在IoC容器或父容器，并且url是public的或提供了setter
...
> 如上，@Value也可以加在setter上，和直接加在成员变量上一样（底层也是使用setter注入，可以省略显示的setter声明）。
...
不再使用context:property-placeholder方式了，使用以下两种方式：
...
以上几种方式也都可以注解到setter上（直接注解到成员时，底层会创建setter，所以可以缺省。但XML方式不能省）。另外，方法名不一定是setUserDao，可以任意，因为是**按形参注入**的。
...
public void test() {
    ...
    System.out.println("第2次获取多例Bean...");
    System.out.println(context.getBean("prototypeBean"));
}
...
Bean处理器。在Bean初始化前后，**非侵入**的对bean进行增强。可以增强指定的bean，也可以增强所有bean。
