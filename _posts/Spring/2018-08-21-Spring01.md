...
常和配置文件配合使用，作用于成员变量、方法上、方法/构造参数上，用于注入指定默认值。

常和${}和#{}配合使用
- #{}：获取容器中的bean或bean属性（Test能访问到dataSource的url属性，同一/父容器中的bean）
- ${}：获取配置文件的k-v对
...
这里Test要能访问到jdbc.username和dataSource.url，即：
- Spring引入了jdbc.properties文件，可以获取到jdbc.username键值对
- dataSource也在Test所在IoC容器，或其父容器，并且url是public的或提供了setter

另外，@Value也可以加在setter上
