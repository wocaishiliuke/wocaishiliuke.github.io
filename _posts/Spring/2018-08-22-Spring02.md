...
**Spring AOP优先选择接口代理（JDK动态代理），对所有实现的接口都进行代理；如果目标对象没有实现任何接口，才会对类进行代理（CGLIB动态代理）**
...
在tx:annotation-driven、aop:aspectj-autoproxy和aop:config上使用proxy-target-class="true"，都将强制使用CGLIB代理。
