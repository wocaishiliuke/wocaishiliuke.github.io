...
> - 不同的参数个数、参数类型、是否使用@Param、接参方式等，会产生多种情况，现将这些情形列举下来
...
> - 下面是以单参数、多参数的传参形式分开讨论，因为Mybatis对这两种情况的处理方式不同
...
> 查看ParamNameResolver的getNamedParams()方法，找到mybatis的封装规律

- 1.如果传递的是单参数，且没有使用@Param，则直接将该单参数传入SqlSession的对应方法
- 2.如果传递的不是单参数或者使用了@Param注解，则会将对应的参数转换为一个Map进行传递。具体规则如下：
    - 2.1先会把对应的参数按照顺序以param1、param2、paramN这样的形式作为Key存入目标Map中
        + 2.2.1 如果参数是以@Param注解命名的参数，还会按顺序以@Param指定的名称作为Key存入目标Map中
        + 2.2.2 如果参数不是以@Param注解命名的，还会按顺序以arg0、arg1、argN这样的形式作为Key存入目标Map中(之前的版本是0、1、N)

> 2.2.1和2.2.2可能会同时出现，即只有部分参数加以@Param
