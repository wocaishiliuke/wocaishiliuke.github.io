...
nginx提供了多系统支持，这里只给出Ubuntu 18.04上stable版的preBuild package安装方式。其他系统或版本或安装方式可参考[官方原文](http://nginx.org/en/docs/install.html)。

> 这里翻译参考的是[nginx商业官网的安装文档](https://docs.nginx.com/nginx/admin-guide/installing-nginx/installing-nginx-open-source/)，其实和[nginx开源的安装说明](http://nginx.org/en/docs/install.html)大体相同
...
这里采用第一种方式：预编译包安装，并且只介绍Ubuntu中的预编译包安装。

> 预编译包方式支持的Linux版本参考[官网说明](https://nginx.org/en/linux_packages.html)

## 3.预编译包Ubuntu版安装

nginx提供了14.04-18.10的安装包。该安装包可以从以下方式获取：

- default Ubuntu repository：Ubuntu默认仓库。方便，但往往不是最新版的
- nginx.org官方仓库：需要设置apt-get源，之后安装的就是最新版了
...
- mainline表示会获取最新的mainline版本源，不带mainline表示获取最新的stable版本源
...
源码安装这里不做翻译，有需要[参考官网](http://nginx.org/en/docs/configure.html)
...
**nginx有一个master进程和多个worker进程。主进程的主要作用是读取配置和维护工作进程，工作进程才是处理请求的**。nginx引入event-based模型和OS-dependent机制，以完成worker进程间请求的高效分发。worker进程数在配置文件中指定，可以是固定配置（数），也可以配置成自动调整为可用CPU core数
...
**重载配置**：一旦master进程收到重载配置的指令，它会先检查配置文件的语法是否合法，然后尝试应用新配置。如果成功，master进程会开启new worker processes，并发送消息到old worker processes让他们关闭。old worker processes收消息后会停止接收新连接，并在处理完当前的requests后exit。如果尝试应用新配置失败，master进程会回滚配置，继续使用原有配置工作。
...
更多关于发送信号到nginx，可参考[Controlling nginx](https://nginx.org/en/docs/control.html)。


## 2.配置文件结构

nginx由modules模块组成，这些modules由配置文件中的指令控制。指令分为simple directives简单指令和block directives指令块。简单指令由name和parameters组成，使用空格分割，并以分号（;）结束。指令块的结构同简单指令，但它是以一组大括号（{}）包围的additional instructions结尾的，而不是分号。如果一个指令块的{}中有其他指令，则称之为context（如：events、http、server和location）

配置文件里不在任何context中的指令，可看是做在main context中。events和http指令在main context中，server指令在http中，location指令在server中
...
一般配置文件中包含多个server块，由他们监听的端口和server name来区分。一旦nginx决定了由哪个server来处理请求，nginx会对请求头中的URI和server块中的location块中的参数进行测试。
...
该配置已经配置了一个server虚拟主机，监听80端口，本地可以使用http://localhost/访问。如果想响应http://localhost/images/example.png请求，需要本地准备/data/images/example.png文件。如果找不到，nginx会返回404。其他不是以/images/开头的请求会被映射到/data/www目录，如请求http://localhost/some/example.html，nginx会返回/data/www/some/example.html文件。

启动（未启动时）或重载配置文件，使新配置生效。
...
> 本地测试时，虽然/etc/nginx/nginx.conf中没有配location，但在include的/etc/nginx/conf.d/*.conf;中包含了一个default.conf，在该配置中配置了两个location，用来映射/usr/share/nginx/html中的欢迎页和50x页。需要注释掉该server再测试
...
将nginx配置成一个代理服务器是种常用方式。代理服务器用来接收request，转发给被代理服务器，并从它们取回Response，响应给客户端。

这里我们将配合一个基本的proxy server：图片请求使用本地文件响应，其他请求转发给proxied server。该示例中所有的服务器都在一个nginx实例中定义
...
然后，使用上一节中的server块（Serving Static Content），将其修改为一个proxy server：在第一个location块中，放置一个proxy_pass指定，明确协议、（站点）名称、被代理服务器的端口
...
当nginx选择一个location来处理request时，它会先检查指定前缀的location块，并记住最长prefix（最匹配）的location，然后才check正则表达式。如果有正则表达式匹配上，nginx会选择这个正则location。否则就选择之前记住的最匹配location。
...
[该部分文档](https://docs.nginx.com/nginx/admin-guide/)位于nginx商业网站上。部分内容和nginx.org文档中的重复。且很多章节将nginx和nginx plus混合在一起说明，内容较多较乱，会持续更新...

## Installing NGINX and NGINX Plus

该部分官网提供了nginx和nginx plus的各种安装方式和场景，这里不再翻译。上面已经翻译了nginx在Ubuntu下的package安装方式

## Basic Functionality

#### Controlling NGINX Processes at Runtime

该部分描述运行时NGINX启动的进程processes，以及如何控制它们
...
工作进程才是实际处理请求的。nginx依赖OS-dependent机制，以完成worker进程间请求的高效分发。工作进程数在nginx.conf中使用worker_processes指令指定，可以设置成固定值，也可以配置成自动调整为可用CPU core数

###### Controlling NGINX

为了重载配置，可以停止或重启NGINX，或发送信号到master process。可以使用nginx命令（调用NGINX可执行文件）和-s参数发送指令。
...
NGINX和NGINX Plus和其他应用相似：都是使用基于文本text‑based、有特定格式的配置文件。默认配置文件名是nginx.conf，NGINX Plus的是放在/etc/nginx目录，NGINX的存在位置，取决于用于安装nginx的包管理器package system和操作系统operating system，通常在/usr/local/nginx/conf，/etc/nginx或/usr/local/etc/nginx

###### Directives

配置文件由指定directives和参数parameters组成。 简单(单行）指令以分号结尾。其他指令像容器一样使用大括号将相关指令汇成一组，通常称之为指令块。简单指令示例如下：
...
为了容易维护配置文件，推荐分割配置成一组feature‑specific的配置文件，放在/etc/nginx/conf.d目录，并在主配置文件nginx.conf中使用include指令引入这些feature‑specific配置文件。
...
有一些顶级指令，被称为context，会聚集一些不同通信方式traffic types的指令:
...
在每个处理通信的context中，可以引入一个或多个server块，定义虚拟主机，来控制请求的处理。server context中可以引入的指令根据通信类型的不同而不同。

对于HTTP通信来说（http context），每个server指令控制着，对特定域名或IP资源的请求处理。一个server块中的一个或多个location context，定义了如何处理匹配的指定URIs的请求

对于mail和TCP/UDP通信（mail和stream context），每个server指令控制着，对特定TCP端口或UNIX socket的通信处理
...
继承。一般来说，子context，包含在另一个context（父context），会继承上级父context中的配置指令。一些指令可以出现在多个context中。所以可以通过覆写override某指令，来覆盖父context中的对应指令配置。如proxy_set_header指令。

###### Reloading Configuration

对配置文件的修改要生效，就必须reload。可以restart nginx进程，也可以发送reload信号（不会打断当前请求的处理）。更多可查看上述Controlling NGINX Processes at Runtime小节
...
想要使用nginx或nginx+将http请求负载均衡到一组虚拟主机上，就需要先使用[upstream](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#upstream)指令定义组，该指令位于[http](https://nginx.org/en/docs/http/ngx_http_core_module.html?#http) context中。

组中的servers使用[server](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#server)指令进行的配置（**不要和定义虚拟主机的server块混淆了**）。如下列配置，定义了一个名为backend的upstream组，包含了3个server配置（可能不止3个实际主机）。其中两个运行同一个应用的实例，另一个作为备用server。

为了传递请求到server组（upstream），将组名指定到[proxy_pass](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_pass)指令（或其他协议的fastcgi_pass、memcached_pass、scgi_pass、uwsgi_pass指令），下面定义了一个运行在nginx上的虚拟主机，会将所有请求传递到backend upstream。
...
Requests均匀的在servers间分配，[server weights](https://docs.nginx.com/nginx/admin-guide/load-balancer/http-load-balancer/#weights)会被考虑。默认值（没有对应的指令）:
...
请求会被发送到最少活跃连接数的server，server weights也会被考虑：
...
由客户端IP来决定请求发送到哪个server。这种情况下，IPv4的前3个octet或者整个IPv6用来计算哈希值。该方式保证了，相同IP的请求由同一台sever响应，除非该server不可用。
...
如果其中一台server需要暂时从负载均衡轮转中移除，可以使用[down](https://nginx.org/en/docs/http/ngx_http_upstream_module.html?#down)参数标记它，是为了保持当前客户端IP的hash。原本将由该server处理的请求，交由组内的下一个server处理。
...
由用户定义的key来决定请求发送到哪个，key可以是string、变量、或两者结合。例如key可能是资源IP和端口对、或URI：
...
每个请求都随机传递到某个server。如果指定了two参数，首先NGINX会随机（考虑权重）选取2台server，然后再使用下列方法选取一个server:

- least_conn：最少活跃连接数
- least_time=header (NGINX Plus)：从server接收到响应header的最小平均时间($upstream_header_time)
...
Random算法应该应用在分布式环境，多个load balancers同时传递request到同一组backends。对于load balancer has a full view of all requests的环境，可以使用其他负载均衡算法，如RR、least connections、least time。

> Note：当配置除RR以外的方法时，要将相应的指令（hash、ip_hash、least_conn、least_time、r random）放在upstream内的server前。


###### Server Weights

默认情况下，nginx根据server的权重，使用RR算法分发请求。server的[weight](https://nginx.org/en/docs/http/ngx_http_upstream_module.html?#weight)参数用来设置该server的权重，默认值1。
...
上例中，backend1.example.com权重5，其他两个默认权重是1。其中192.0.0.1被标记为backup server，它不接收请求，除非其他两个都不可用的时候。该配置下，每6个request，5个被发送到backend1.example.com，1个被发送到backend2.example.com。

###### Server Slow-Start

server的slow-start慢启动特性，可以保护一个最近恢复的server，不被connections压垮，否则可能连接超时，导致该server被再次标记成failed。

在NGINX Plus中，slow‑start允许一个upstream server，在恢复或变得可用时，从0逐步恢复它的weight到正常值。可以在server指令中使用[slow_start](https://nginx.org/en/docs/http/ngx_http_upstream_module.html?#slow_start)参数来实现：
...
时间值（30s）指定了，NGINX Plus将该server的连接值，斜坡上升到最大值的时间。
...
[Session persistence](https://www.nginx.com/resources/glossary/session-persistence/) 会话保持，是指NGINX Plus识别用户Session，并将给定Session的所有请求路由到同一个主机upstream server。
...
该例中，其中一个主机会通过在响应中设置EXAMPLECOOKIE Cookie，创建一个Session。

必须参数create指定一个变量，用以表明如果创建一个新Session。该例中，根据主机发送的EXAMPLECOOKIE Cookie创建。
...
对于NGINX Plus，通过使用[max_conns](https://nginx.org/en/docs/http/ngx_http_upstream_module.html?&#max_conns)参数指定最大连接数，来限制某个upstream server的连接数。

当达到最大连接数时，如果使用了[queue](https://nginx.org/en/docs/http/ngx_http_upstream_module.html?#queue)指令设置了队列中同时存在的最大请求数，则后续的request会排队等待被处理。
...
注意，如果其他worker进程中有空闲的keepalive连接，max_conns的限制会被忽略。这就导致了，多个worker进程共享的内存中，一个server的连接数可能会超过max_conns值。
...
当在upstream中使用zone指令时，upstream组的配置被保存在内存中，被所有的worker进程共享。这种情况下时动态可配置的，因为所有worker进程
