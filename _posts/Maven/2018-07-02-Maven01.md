...
>先使用jetty插件进行发布：右键pom.xml文件，maven build中执行jetty:run（或在terminal中mvn jetty:run）
...
> - tomcat7-maven-plugin同样有tomcat7:run goal可以绑定到package phase上（或在terminal中mvn tomcat7:run）
