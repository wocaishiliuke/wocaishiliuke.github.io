...
#### 3.IDE中集成maven
> 在eclipse4.0+中，preferences中已经有maven选项，不需要安装maven插件；较低版本的eclipse需要安装m2e插件，注意插件与eclipse的版本对应

在preferences-maven中
- Installations中指定自己安装的maven目录
- 指定Settings文件和repository位置
- 新建的maven项目显示默认使用JDK1.5，修改的方式有3种
    + 1.单个项目修改JRE System Library（但update后无效）
    + 2.在项目pom.xml中设置compiler插件
    + 3.修改settings.xml，在profiles中添加profile进行设置（全局，推荐）
...
- eclipse中操作maven工程：run as--Maven build...--在goals中输入命令，实例：clean compile（相当于mvn clean compile）
...
> 例如执行package命令时，同一周期的compile和test会先自动依次执行，但不会执行clean
- 尽管phase对应周期中的某个特定阶段，但对于不同的打包方式，phase的具体处理方式可能会不同，所以通过绑定phase和插件的goal来解决这个问题
...
> - clean和site两个周期的phase，对于不同的打包方式都是相同的


### 6.插件和生命周期的绑定
> - 不同的打包方式，（default）周期可能包含不同的phases，并且同一个phase对于不同的打包方式，具体的处理方式也会不同，所以通过绑定phase和插件的goal来解决这个问题
...
- 绑定有两种方式：1.指定packaging方式，2.自定义绑定

#### 1.指定packaging
这是最常用的方式，通过在POM中指定packaging来实现，不指定默认是jar。maven对不同打包方式内置了一些绑定
...
- default周期对于不同的打包方式，包含不同的phases，且同一phase在不同打包方式时的处理方式也会不同。所以maven在定义default周期的phase时，没有直接绑定插件，而是在default-bindings.xml中根据打包方式分开绑定。以jar为例
...
#### 2.自定义绑定
> 将maven-source-plugin的jar-no-fork目标绑定到default生命周期的package阶段。这样，以后在执行mvn package命令打包项目时，除了执行package内置的goal外，还会执行源代码的打包，生成如：helloworld-model-0.0.1SNAPSHOT-sources.jar形式的源码包
...
- **依赖冲突**
> 指一个工程中，引入了不同版本的同一个jar
    - 1.短路优先
        - A依赖B，B依赖C，C依赖X（版本1）；A依赖D，D依赖X（版本2）。则A依赖（版本2）的X
    - 2.依赖路径相同时，按声明顺序优先
        - A依赖B，B依赖C（版本1）；A依赖D，D依赖C（版本2），在A的pom中B声明在D之前。则A依赖（版本1）的C
...
创建聚合工程aggregator（可以不需要src目录，只有pom.xml也可以，主要打包方式为pom）
...
> 此时对aggregator执行指令，如mvn clean install，则会对a，b，c进行清理、编译、测试、打包、安装（对aggregator只进行了clean和install）
...
使用eclipse的maven-archetype-webapp插件创建
...
- 3.项目调整
    + 3.1 archetype-webapp插件生产的项目只有src/main/resources目录，需要手动添加目录满足maven工程目录的要求
        * 1.方式1：修改Java Build Path中JRE System Library为当前Workspace default JRE，update后即可显示，缺省的再手动创建
        * 2.方式2：在Navigator视图中手动创建folder目录（其他视图创建source folder可能会提示已存在），然后update即可
    + 3.2 确保project facets中包括了Dynamic Web Module
    + 3.3 确保Java Build Path中确保source的输出目录在target/classes和target/test-classes下
    + 3.4 在Deployment Assembly中，把test相关的source排除在发布部署外

- 4.发布

先使用jetty插件进行发布，maven build...执行jetty:run
```
[INFO] ------------------------------------------------------------------------
...
访问http://0.0.0.0:8080 即可

- 5.将jetty:run绑定到package阶段

> 将jetty-maven-plugin:9.4.5.v20170502:run绑定到default周期的package阶段，这样在执行mvn package时，会同时执行jetty:run这个goal，从而运行jetty服务
...
> maven build--执行clean package。除了执行package相关的goal外，还会执行jetty:run，启动jetty服务
...
> tomcat7-maven-plugin同样有tomcat7:run goal

访问http://localhost:8080/web-demo 即可
